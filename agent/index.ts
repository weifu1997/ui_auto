...
import { buildPickerCandidates, pickerInjectionScript, type PickerCandidate, previewPickerCandidate as highlightPickerCandidate } from "../server/picker-core.ts";
...
async function startDebugSession(socket: WebSocket, identity: AgentIdentity, payload: DebugStart) {
  ...
  try {
    ...
    await page.exposeBinding("autoflowDebugPickerCapture", async (_source, payload) => {
      ...
      const candidates = await buildPickerCandidates(runtime.page, target, runtime.testIdAttribute, Object.values(runtime.secrets));
...
async function enablePicker(socket: WebSocket, runtime: DebugRuntime) {
  // Inject the picker as a plain-JS string (see picker-core); tsx rewrites TS declarations
  // with an `__name` helper that is not available inside page.evaluate's serialized function.
  const script = pickerInjectionScript(runtime.testIdAttribute);
...
async function previewPickerCandidate(socket: WebSocket, runtime: DebugRuntime, candidate: PickerCandidate, captureId: string, candidateIndex: number) {
  const count = await highlightPickerCandidate(runtime.page, candidate, runtime.testIdAttribute);
