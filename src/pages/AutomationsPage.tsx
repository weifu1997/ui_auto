  ...
  return (
    <>
      ...
      <div className="automation-grid">
        ...
        <section className="surface automation-panel"><div className="panel-heading"><div><h2>投递记录</h2><span>不包含密钥和运行参数</span></div></div><div className="run-filter"><Space wrap><span>状态</span><Select aria-label="投递状态" value={deliveryStatus} style={{ width: 110 }} onChange={(value) => { setDeliveryPage(1); setDeliveryStatus(value); }} options={[{ value: "", label: "全部" }, { value: "pending", label: "pending" }, { value: "retrying", label: "retrying" }, { value: "delivering", label: "delivering" }, { value: "delivered", label: "delivered" }, { value: "failed", label: "failed" }]} /><span>通道</span><Input aria-label="投递通道" value={deliveryChannel} style={{ width: 160 }} placeholder="通道名称" onChange={(event) => { setDeliveryPage(1); setDeliveryChannel(event.target.value); }} /><span>开始</span><Input type="date" aria-label="投递开始日期" value={deliveryFrom} onChange={(event) => { setDeliveryPage(1); setDeliveryFrom(event.target.value); }} /><span>结束</span><Input type="date" aria-label="投递结束日期" value={deliveryTo} onChange={(event) => { setDeliveryPage(1); setDeliveryTo(event.target.value); }} /></Space></div><Table size="small" loading={loading} rowKey="id" dataSource={deliveries} pagination={{ current: deliveryPage, pageSize: 8, total: deliveryTotal, showSizeChanger: false, onChange: (nextPage) => setDeliveryPage(nextPage) }} columns={[{ title: "通道", dataIndex: ["channel", "name"] }, { title: "状态", dataIndex: "status", width: 95, render: (status: PlatformDelivery["status"]) => <Tag color={status === "delivered" ? "success" : status === "failed" ? "error" : "processing"}>{status}</Tag> }, { title: "时间", dataIndex: "createdAt", width: 160, render: (value: string) => new Date(value).toLocaleString() }]} locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚无投递记录" /> }} /></section>
      </div>
