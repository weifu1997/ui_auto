...
/**
 * 本地采集通道：未连接平台账户 / 项目未导入时，「从页面获取」走本机执行服务
 * （server/index.ts 的 local-picker 能力），复用与平台通道一致的候选/回填语义。
 */
export function LocalElementPickerPanel({
  ...
  // 显式打开（或复用）当前环境的本地调试浏览器；已有会话时服务端复用，不会重复打开。
  ...
  if (!effectiveEnvironment) {
    return <Alert type="info" showIcon title="请先在元素表单中选择默认验证环境后再从页面获取。" />;
  }
