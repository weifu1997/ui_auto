...
async function launchBrowser(headless?: boolean) {
  ...
  // 环境未配置窗口模式时默认无头（后台执行），与 managed 执行器默认一致。
  return chromium.launch({ headless: headless ?? true });
}
...
async function executeStep(
  ...
  if (action !== "打开页面" && element && page.url() === "about:blank") {
    const target = environmentUrl(input.environment.baseUrl, element.path || "/");
    await page.goto(target, { waitUntil: "domcontentloaded", timeout: input.environment.timeout * 1000 });
    publish(task, "log", { level: "info", message: `自动打开页面：${target}` });
  }
  ...
  try {
    ...
    if (action !== "打开页面" && error instanceof Error && error.message.includes("Timeout")) {
      throw new Error(
        `步骤「${action}」超时（${timeout / 1000}s）。当前页面：${page.url()}。请确认流程已通过「打开页面」步骤打开目标页面，且元素定位与页面内容一致。原始错误：${error.message}`,
      );
    }
    throw error;
  }
...
async function executeRun(task: Task, request: RunRequest) {
  ...
  try {
    ...
    persistTask(task);
    browser = await launchBrowser(input.environment.headless);
...
async function executeValidation(task: Task, request: ValidationRequest) {
  ...
  try {
    task.browserState = "launching";
    persistTask(task);
    browser = await launchBrowser(environment.headless);
    task.browser = browser;
    observeBrowser(task, browser);
    context = await browser.newContext();
    task.context = context;
