...
import type {AgentRecord, AuthUser, DeliveryStatus, LocatorCandidate, NotificationChannelType, PlatformApi, RevisionStatus, Role, ValidatedNotificationTarget} from "./platform-core";
  ...
  const handle = routeHandler(async (request, response, url) => {
    ...
    if (publicWebhook && request.method === "POST") {
      ...
      if (url.pathname === "/api/auth/login" && request.method === "POST") {
        ...
        const user = services.database.prepare(`SELECT id, email, name FROM platform_users WHERE email = ?`).get(email) as AuthUser | undefined;
        ...
        sendJson(response, 200, services.createAuthSession(user));
        return true;
      }
      ...
      if (revisionRoot) {
        ...
        if (request.method === "POST") {
          ...
          services.requireChromiumEnvironment(asRecord(body.environment));
      ...
      if (revisionAction && request.method === "POST") {
        ...
        const { project } = services.requireProjectAdmin(projectId, user.id);
        const revision = services.database.prepare(`SELECT id, status, flow_id, environment_id FROM flow_revisions WHERE id = ? AND project_id = ?`).get(revisionId, projectId) as { id: string; status: RevisionStatus; flow_id: string | null; environment_id: string | null } | undefined;
        ...
        try {
          ...
          services.database.prepare(`UPDATE flow_revisions SET status = 'published', published_at = ? WHERE id = ?`).run(now(), revisionId);
          services.database.exec("COMMIT");
        } catch (error) {
          services.database.exec("ROLLBACK");
          throw error;
        }
        services.audit(project.workspace_id, { type: "user", id: user.id }, action === "publish" ? "flow_revision.published" : "flow_revision.rolled_back", { type: "flow_revision", id: revisionId }, {}, projectId);
      ...
      if (datasetVersionRoot) {
        ...
        const { project } = services.requireProjectRole(projectId, user.id, request.method !== "GET");
        const dataset = services.database.prepare(`SELECT id, name FROM datasets WHERE id = ? AND project_id = ?`).get(datasetId, projectId) as { id: string; name: string } | undefined;
      ...
      if (scheduleAction && request.method === "POST") {
        ...
        const { project } = services.requireProjectAdmin(projectId, user.id);
        const schedule = services.database.prepare(`SELECT id, revision_id, environment_id, dataset_version_id FROM schedules WHERE id = ? AND project_id = ?`).get(scheduleId, projectId) as { id: string; revision_id: string; environment_id: string; dataset_version_id: string | null } | undefined;
      ...
      if (webhookRoot) {
        ...
        const { project } = request.method === "GET" ? services.requireProjectRole(projectId, user.id) : services.requireProjectAdmin(projectId, user.id);
        if (request.method === "GET") {
          const triggers = services.database.prepare(`SELECT id, revision_id, environment_id, dataset_version_id, name, enabled, created_at, last_triggered_at FROM webhook_triggers WHERE project_id = ? ORDER BY created_at DESC`).all(projectId) as Array<{ id: string; revision_id: string; environment_id: string; dataset_version_id: string | null; name: string; enabled: number; created_at: string; last_triggered_at: string | null }>;
      ...
      if (notificationChannelRoot) {
        ...
        services.requireWorkspaceRole(workspaceId, user.id, request.method !== "GET");
        if (request.method === "GET") {
          const channels = services.database.prepare(`SELECT id, name, channel_type, enabled, created_at, updated_at FROM notification_channels WHERE workspace_id = ? ORDER BY name`).all(workspaceId) as Array<{ id: string; name: string; channel_type: NotificationChannelType; enabled: number; created_at: string; updated_at: string }>;
      ...
      if (notificationSubscriptions) {
        ...
        if (request.method === "PUT") {
          ...
          const channel = services.database.prepare(`SELECT id FROM notification_channels WHERE id = ? AND workspace_id = ?`).get(body.channelId, project.workspace_id) as { id: string } | undefined;
      ...
      if (validationRoot) {
        ...
        if (request.method === "POST") {
          ...
          services.requireSameOriginElementPath(environment, element);
          const agent = services.candidateAgent(projectId, body.environmentId);
          if (!agent || !services.sockets.has(agent.id)) throw new PlatformError(409, "AGENT_UNAVAILABLE");
          const validation = { id: randomUUID(), createdAt: now() };
          services.database.prepare(`INSERT INTO element_validations (id, project_id, environment_id, agent_id, status, element_snapshot, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, 'queued', ?, ?, ?, ?)`)
            .run(validation.id, projectId, body.environmentId, agent.id, json(element), user.id, validation.createdAt, validation.createdAt);
          const persisted = services.elementValidationById(validation.id);
          if (!services.sendAgent(agent.id, services.agentValidationPayload(persisted))) {
            services.database.prepare(`UPDATE element_validations SET status = 'failed', error = 'AGENT_UNAVAILABLE', updated_at = ? WHERE id = ?`).run(now(), validation.id);
            throw new PlatformError(409, "AGENT_UNAVAILABLE");
          }
          ...
          sendJson(response, 202, { validation: services.elementValidationById(validation.id) });
          return true;
        }
      }
      ...
      if (platformRunDetail) {
        ...
        services.requireProjectRole(projectId, user.id, request.method !== "GET");
        ...
        if (request.method === "POST" && action === "cancel") {
          ...
          if (lease) {
            services.sendAgent(lease.agentId, { type: "run.cancel", leaseId: lease.id, runId: run.id });
          }
