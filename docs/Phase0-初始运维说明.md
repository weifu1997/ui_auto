...
Windows 部署入口是 `scripts/install.ps1`，它会准备 `D:\AutoFlow` 下的 `app`、`data`（包括 `data\artifacts`）、`logs`、`backups`、`browsers` 和 `runtime` 目录，渲染 WinSW 配置并启动服务。运行前应由主机管理员确认服务账号、目录 ACL 和密钥保存方式；这些安全控制尚未在仓库内实现或验证。
...
3. 对应用回归使用 `scripts/rollback.ps1`；对数据问题只在已验证备份下使用 `scripts/restore.ps1`。
