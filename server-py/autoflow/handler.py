...
def create_platform_router(services: PlatformServices) -> APIRouter:
    ...
    @router.api_route("/api/health", methods=["GET"])
    def worker_health() -> Response:
        return _send(Response(), 200, {"ok": True})

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions",
        methods=["POST"],
    )
    async def local_picker_session_create(
        ...
        if not environment_id:
            raise PlatformError(400, "LOCAL_PICKER_INPUT_INVALID")
        environment = _recording_environment(services, project_id, environment_id)
        services.require_chromium_environment(environment)
        session = services.local_picker.create_session(
            project_id, environment, start_url=start_url or ""
        )
    ...
    async def local_picker_sessions_list(
        request: Request, project_id: str
    ) -> Response:
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        sessions = services.local_picker.list_sessions(project_id)
        return _send(Response(), 200, {"sessions": sessions})

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions/{session_id}/picker/enable",
        methods=["POST"],
    )
    async def local_picker_session_enable(
        request: Request, project_id: str, session_id: str
    ) -> Response:
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        session = services.local_picker.enable_picker(session_id)
        if session is None:
            raise PlatformError(404, "LOCAL_PICKER_SESSION_NOT_FOUND")
        return _send(Response(), 200, {"session": session})

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions/{session_id}/picker-captures",
        methods=["GET"],
    )
    async def local_picker_captures(
        ...
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        captures = services.local_picker.get_captures(session_id)
        return _send(Response(), 200, {"captures": captures})

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions/{session_id}/picker-captures/{capture_id}/preview",
        methods=["POST"],
    )
    async def local_picker_capture_preview(
        request: Request, project_id: str, session_id: str, capture_id: str
    ) -> Response:
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        body = await request.json()
        ...
        result = services.local_picker.preview_candidate(
            session_id, capture_id, candidate_index
        )
        return _send(Response(), 200, result)

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions/{session_id}/picker-captures/{capture_id}/confirm",
        methods=["POST"],
    )
    async def local_picker_capture_confirm(
        request: Request, project_id: str, session_id: str, capture_id: str
    ) -> Response:
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        ...
        result = services.local_picker.confirm_candidate(
            session_id, capture_id, candidate_index, name=name
        )
        if result is None:
            raise PlatformError(404, "LOCAL_PICKER_CAPTURE_NOT_FOUND")
        return _send(Response(), 200, result)

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions/{session_id}/commands",
        methods=["POST"],
    )
    async def local_picker_session_command(
        request: Request, project_id: str, session_id: str
    ) -> Response:
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        ...
        if not isinstance(body, dict):
            body = {}
        command = _text(body.get("command")).strip()
        if command == "stop":
            session = services.local_picker.stop_session(session_id)
            if session is None:
                raise PlatformError(404, "LOCAL_PICKER_SESSION_NOT_FOUND")
            return _send(Response(), 200, {"ok": True, "session": session})
        raise PlatformError(400, "LOCAL_PICKER_COMMAND_INVALID")

    @router.api_route(
        "/api/projects/{project_id}/local-picker/sessions/{session_id}/screenshot",
        methods=["GET"],
    )
    async def local_picker_screenshot(
        ...
        user = services.session_user(dict(request.headers))
        services.require_project_role(project_id, user.id)
        data_url = services.local_picker.get_screenshot(session_id)
        if not data_url:
            raise PlatformError(404, "LOCAL_PICKER_SCREENSHOT_NOT_READY")
        import re
        match = re.match(r"data:image/png;base64,(.+)", data_url)
        if not match:
            raise PlatformError(500, "LOCAL_PICKER_SCREENSHOT_INVALID")
        png_bytes = _b64.b64decode(match.group(1))
