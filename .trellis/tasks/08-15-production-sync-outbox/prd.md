...
- [ ] 保存任一资源后立即导航、刷新或关闭再打开，修改最终仍可在服务端恢复。
- [ ] 断网/5xx 恢复后无需再次编辑即可自动同步。
- [ ] 并发 409 时本地草稿可恢复且不会静默覆盖远端；用户能明确选择刷新远端或重新提交。
- [ ] production/auth-required 场景下新增或更新自动化测试，覆盖导航、刷新、5xx、409 和重启重放。
