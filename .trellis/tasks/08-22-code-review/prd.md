...
- [ ] PRD 明确记录最终审查范围与排除项。
- [ ] 审查报告覆盖至少：正确性、安全性、可维护性、测试缺口和跨层一致性。
- [ ] 审查报告覆盖 `src/`、`server-py/`、`scripts/`、`deployment/` 和测试配置。
- [ ] 每个非信息级发现都有严重程度、文件/行号证据、影响说明和建议动作。
- [ ] 报告区分确认问题、待验证风险和误报排除结论。
- [ ] 记录已执行的质量门禁、结果摘要和未执行门禁的原因。
