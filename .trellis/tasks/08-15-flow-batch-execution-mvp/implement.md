...
- Phase 0 是硬前置；单流程 revision 选择未修复前不得开放 batch UI。

## Phase 0: Correct Single-Flow Revision Selection

- [ ] 扩展 revision resolver 按 `projectId + flowId + environmentId` 查询最新 published revision，同时保留显式 revisionId 路径。
- [ ] 修改手工单运行 API 接受 `flowId`，并在 revisionId/flowId/environmentId 之间做一致性校验。
- [ ] 修改 FlowsPage 列表运行传当前 flow id；修改编辑器运行到此步骤传当前 flow id 或已解析 revision id。
- [ ] 搜索所有 `createPlatformRun` 调用方并逐一确认，不遗漏 RunDetail retry 或其他入口。
- [ ] 修改当前把“不含 revisionId”视作正确行为的 E2E 断言，新增流程 A/B revision 回归。

**Gate**：保存 A 后保存 B，分别点击 A/B 时服务端 snapshot.flow.id 与所选 flow 一致；现有显式 revision 调用兼容。
...
- [ ] Retry 复用 create/preflight helper，解析当前最新 revision，不复制旧 snapshot。
...
1. `fix(runs): resolve revisions by selected flow`
2. `refactor(runs): extract reusable run construction`
3. `feat(batch): persist idempotent run batches`
4. `feat(batch): cancel and retry failed items`
5. `feat(flows): add batch run selection`
6. `feat(runs): display batch progress and actions`
7. `test(batch): cover recovery and execution closure`
