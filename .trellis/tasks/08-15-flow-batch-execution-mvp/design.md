...
本设计依赖独立 P0 任务 `08-15-flow-revision-selection-correctness`。Batch 只消费已经验收的 flow-scoped revision resolver，不在本任务复制或重做单流程修复。
...
## Consumed P0 Revision Contract

P0 提供等价于以下输入能力的服务层 resolver：
...
列表按 `created_at DESC` 排序（2026-08-16 已确认），保证运行中的批次跨页位置稳定。列表返回 `{ batches, total, page, pageSize }`。详情返回 `{ batch, runs }`，runs 按 `batch_item_index ASC`。批次列表不把所有执行 snapshot 嵌入响应。
...
服务端要求原 batch 终态，提取 failed/canceled 子项，使用原 environmentId 和每个子项原 run 的 revision 快照（允许 superseded，复用 P0 的重试解析），走与 create 相同的事务/入队 helper，设置 `retry_of_batch_id`，并为每个新 run 记录 `retry_of_run_id`。
